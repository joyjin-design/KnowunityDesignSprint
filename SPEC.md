...
- **Dimmed** (`showBottomSheetBackground`) while the sheet is open — the general sheet rule; the verdict sheet (screen 1) and the mic-off sheet (screen 5) are the two documented exceptions, not this one, and **stay** the only two (decided 2026-09-17). This screen's dimmed caption also measures under the 4.5:1 body-text floor (~2.82:1) — same root cause as screen 5's fix — but here that's intentional, not a defect: once the permission sheet is up, the sheet is the thing to act on, not the headline/body behind it, the same "background recedes, modal is the focus" pattern iOS itself uses. Screen 5's case was different because the transcript/disclaimer behind that sheet is content the student still needs mid-task, not a backdrop. Figma's own "sheet open" frame having the dim off is still an unresolved authoring-gap question, separate from the contrast point.
...
- **States:** after Pass, Partial or Fail — only the concepts the (mocked) judge found missing are bold, on any of the three verdicts. A Pass only needs 2 of 3 concepts (judge rules below), so a Pass can still bold one missed concept, on purpose — it's a chance to double-check, not an error signal (2026-09-16, your call; supersedes this line's own earlier "Success has nothing missing, so nothing's bold" claim, and the walkthrough's step 4 below). Silence has no Why? button (`ResultBtm` doesn't render one for it).
...
- **"Still listening…" retriggered on real silence, 2026-09-16 (your call).** It no longer fires off a recognizer restart or a word count: `LoopScreen.tsx` arms a 3s timer on every interim result from either speech source (live or scripted) and shows the cue the moment that timer elapses with nothing new — real silence, not a proxy for it. It clears the instant a new interim result lands, or the take ends; there's no separate auto-hide any more. (The earlier restart-based trigger, and the spike data point about a 49s take with no restart, are both moot now — restarting recognition on an iOS pause still happens in `webSpeech.ts`, it just isn't what shows the cue.) **Hides on speech starting, not just on transcribed text, 2026-09-16 (your follow-up).** With the real recognizer, waiting for `onInterim` to hide the cue meant it could linger for a beat after the student actually resumed speaking, especially right after a long gap — transcription lags a little behind the sound itself. `webSpeech.ts` now also fires `onSpeechStart` (the Web Speech API's own "speech detected" event, ahead of any words), which `LoopScreen.tsx` uses to hide the cue and rearm the 3s timer immediately, no matter how long the preceding gap was. The scripted stand-in has no equivalent signal (it only knows words, never "sound before words"), so it's unchanged — this only sharpens the live-mic path. Cue's own show/hide is now a 150ms opacity fade (`.stillListening`, "option A" from the animation audit's missed-opportunities list) instead of a hard pop, staying mounted for all of Recording so there's something to transition between.
- **The waveform row is real, 2026-09-16 (your call).** Recording reveals bars from `WAVEFORM_BAR_HEIGHTS` (`LoopScreen.tsx`, transcribed off Figma's own 06Talking → 07KeepTalking → 08Talking-finished progression) on a 200ms clock, appended in their natural Figma order. Bars are grouped as one unit (`.waveformGroup`) whose own `transform: translateX` — recomputed each time a bar is added (`waveformGroupShift`), transitioning smoothly (220ms, `--ease-out-expo`) — keeps the newest bar flush against the row's right edge: since appending never reflows a bar that's already on screen, this single transform is what makes the *whole group* glide left together as it grows, rather than each bar popping in on its own (2026-09-16, your call — "show them as a group ... move from right to left," replacing the earlier per-bar entrance). That clock is gated on the same silence signal that drives "Still listening…": it only advances once at least one interim result has arrived, and pauses the instant 3s of silence is confirmed, so bars trickling in and the "Still listening…" cue are mutually exclusive, never both at once (`webkitSpeechRecognition` exposes no live amplitude, so "sound" here means "an interim result arrived," not a measured volume — same ceiling the old word-count proxy had). The row itself sits 28px in from the screen edge (Figma node `13696:7029`), narrower than the 16px button row below it. Processing holds that same bar count frozen (Figma's own "Processing" frame draws the identical row) and pulses it like a slow equalizer (`animation: waveform-pulse`, 1.6s — matched to the mascot's own `thinking-bob` period so the two loops phase-lock instead of drifting, per-bar stagger) instead of leaving it static.
...
- **Idle-only mic-unavailable snackbar, 2026-09-16 (your call, Figma 13719:8829/9060), narrower than the in-loop-snackbars exclusion above.** Gate already confirms the mic before the loop ever opens, but it can be revoked, or hardware unplugged, after that with nothing tapped since — a real gap the gate's own recheck can't catch. `PrototypeFlow` passively re-asks the mic (the same `requestMic` Gate and Start already use) each time a fresh question's Idle is reached; a non-`granted` result shows `Snackbar` (`variant="Error"`) above the app bar, "Your microphone is not available.", one action "Go back" (copy updated 2026-09-17 to match your Figma edit, node 13719:9060). Idle only — Start's own existing recheck (`handleLoopStart`, the reactive `MicUnavailable` path in the table below) is still what actually stops Recording, so this snackbar never blocks anything, it only warns ahead of the tap. No real OS Settings deep link exists from a web page, so "Go to Setting" sends the student back to Gate's own after-denial Settings-instructions state (screen 6) instead of a second copy of it here; `session` is never cleared by that trip, so Gate's "I've turned on the mic" recheck resumes the same question via `beginSession`'s existing same-node guard, not a fresh session.
...
| Idle, mic unavailable | Idle, plus `Snackbar` (`variant="Error"`) above the app bar: "Your microphone is not available." + "Go back" (2026-09-16, see above; copy updated 2026-09-17) | Same as Idle, plus **Go back** → Gate's after-denial state (screen 6), same question resumed on return |
| Recording | `ButtonVoice state="Recording"` with waveform + "Send" (Figma still says "Stop"; deferred); the `ButtonIcon` beside it swaps from Skip to a discard icon (Phosphor `ArrowCounterClockwise`); Can't talk right now hidden. Transcript streams live (`Filled`, then `Overflow` anchored to the newest text). Above the button row, a bar-waveform grows as a group, gliding right to left as new bars join on the right, while there's sound coming in (2026-09-16 — see below) | **Send** → processing. **Cancel** → idle, take thrown away. **Close** → 01Exam |
...
- **In-loop snackbars**, except the one Idle-only mic-unavailable exception below (screen 10) — recording-time snackbars (e.g. an error mid-take) stay out.
...
4. **Start** → waveform + Send, cancel icon instead of Skip, Can't talk right now hidden. Speak a 2-concept sentence, then pause 3s+ → "Still listening…" appears and nothing is sent. **Send** → button hidden, phrases advance, mascot moves → **Success**. **Why?** → explanation with the third, unhit concept bolded even though this is a Pass (2026-09-16: a Pass only needs 2 of 3, so the miss is still shown — see screen 7's "States" above), mascot and Got it above the sheet, can't be dragged down. **Got it** → Q2, progress 25.
