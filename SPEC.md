...
- **Idle-only mic-unavailable snackbar, 2026-09-16 (your call, Figma 13719:8829/9060), narrower than the in-loop-snackbars exclusion above.** Gate already confirms the mic before the loop ever opens, but it can be revoked, or hardware unplugged, after that with nothing tapped since — a real gap the gate's own recheck can't catch. `PrototypeFlow` passively re-asks the mic (the same `requestMic` Gate and Start already use) each time a fresh question's Idle is reached; a non-`granted` result shows `Snackbar` (`variant="Error"`) above the app bar, "Mic is not available.", one action "Go to Setting". Idle only — Start's own existing recheck (`handleLoopStart`, the reactive `MicUnavailable` path in the table below) is still what actually stops Recording, so this snackbar never blocks anything, it only warns ahead of the tap. No real OS Settings deep link exists from a web page, so "Go to Setting" sends the student back to Gate's own after-denial Settings-instructions state (screen 6) instead of a second copy of it here; `session` is never cleared by that trip, so Gate's "I've turned on the mic" recheck resumes the same question via `beginSession`'s existing same-node guard, not a fresh session.
...
| Idle, mic unavailable | Idle, plus `Snackbar` (`variant="Error"`) above the app bar: "Mic is not available." + "Go to Setting" (2026-09-16, see above) | Same as Idle, plus **Go to Setting** → Gate's after-denial state (screen 6), same question resumed on return |
