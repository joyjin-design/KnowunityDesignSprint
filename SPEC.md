...
| Recording | `ButtonVoice state="Recording"` with waveform + "Send" (Figma still says "Stop"; deferred); the `ButtonIcon` beside it swaps from Skip to a discard icon (Phosphor `ArrowCounterClockwise`); Can't talk right now hidden. Transcript streams live (`Filled`, then `Overflow` anchored to the newest text). Above the button row, a bar-waveform reveals bars one at a time, sliding in from the right, while there's sound coming in (2026-09-16 — see below) | **Send** → processing. **Cancel** → idle, take thrown away. **Close** → 01Exam |
