...
- **Top hairline reconciled with \`bottomSheetVerdict\`.** This set bound no stroke at all, where bottomSheetVerdict draws a 1px top-only \`border/default\` hairline (\`strokeTopWeight\` 1, all other edges 0, \`INSIDE\`) that separates the sheet from the dimmed screen behind it. By your decision that hairline was added to all three variants **in Figma** and mirrored here. Worth knowing: \`border/default\` and \`background/stacking\` both alias \`alpha.light-10\`, so the hairline and the grab handle now render the identical colour — one 10%-white treatment for both, which is what bottomSheetVerdict was already doing.
`;
