...
- **Handle reconciled with \`bottomSheetVerdict\`.** Geometry was already identical to the handle built there (32×4, centred, \`space.150\` from the top, ABSOLUTE), but this set bound its fill to \`background/floating\` — \`#3d3d3d\` at 60%, which composites to roughly \`#323337\` on \`background/surface\` and is nearly invisible — where bottomSheetVerdict uses \`background/stacking\` (white at 10.2%, roughly \`#393a44\`, clearly legible). By your decision the fill was changed to \`background/stacking\` **in Figma** across all four variants, and its 2px radius bound to \`Stroke/Heavy Border\` where it had been an unbound raw \`2\`. The two sheets now agree.
