...
**What it's scoring:** are all the states this project actually requires handled, is the hierarchy of primary vs. secondary action clear, and are the failure/recovery paths designed rather than bolted on. Also whether the student is left with a next step once a session ends, not just an escape hatch mid-turn. Grade against `Voice-ux.md`'s six principles and the states-to-design checklist, and CLAUDE.md's hard rules — not generic UX heuristics.
...
Separate from the six scored dimensions and the hard gates. A real finding — usually from user testing, not code review — that maps to a scope decision already made in `SPEC.md`, `Voice-ux.md`, or `CLAUDE.md`. Log it here instead of scoring it against a dimension: the build isn't wrong for not doing work it was explicitly told not to do this sprint, but the finding shouldn't be lost either.

**Test before filing something here:** does it map to an existing, named scope decision? If yes, it belongs in this section. If no — if it's a real gap in something the build was supposed to do — score it under the dimension it belongs to instead. (Icon legibility, live-transcript bugs, and similar defects in built, in-scope screens are Craft/Structure findings, not scope exclusions, even when a user-testing session is where they surfaced.)

| Finding | Source | Why it's out of scope this sprint |
|---|---|---|
| Thumbs up/down after an incorrect answer felt out of place — confusing to see a positive/negative rating when the answer was already marked wrong | 2nd participant (40s, iPhone 11) | The control is "rendered and unwired, by decision" on all four verdict states (`design-system.md`, `SPEC.md`) — its incompleteness this sprint is already a named call, not an unflagged gap |
| XP scoring unclear — points per question vs. session unclear; suggested variable points by difficulty | 2nd participant | XP is a static, non-functional "2" this sprint by design (`SummaryScreen`, `sprint-context.md`) — the "fabricated metric" problem was already ruled out, not left as a gap |
| Voice-only felt limiting when unable to speak — wanted a working typing/multiple-choice fallback, not just an exit | 2nd participant | The typing turn is out of scope this sprint (CLAUDE.md hard rule) — `TypingPlaceholderScreen` is a deliberate placeholder, not an unfinished fallback |
| Notification dot / bottom-nav order not noticed at a glance on a real device | 1st and 2nd participants | Home screen and exam plan are static exported Figma frames with tap zones, not built components (`SPEC.md` §3) — not gradable UI |
